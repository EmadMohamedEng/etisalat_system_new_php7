...
use App\Models\Contractdepartments;
use App\Models\Questionstranslation;
...
class SiteHelpers {
    ...
    public static function transForm($field, $forms = array(), $bulk = false, $value = '') {
        ...
        switch ($type) {
            ...
            case 'text_date';
                $form = "<input  type='text' name='$field{$bulk}' class='date_lang form-control input-sm' $mandatory value='{$value}'/> ";
                break;
            ...
            case 'select';
                ...
                if ($option['opt_type'] == 'external') {
                    ...
                    /*
                      ...
                      $data = DB::table($option['lookup_table'])->where('entry_by','=',\Auth::user()->id)->get();
                      }
                     */
                    ...
                    foreach ($data as $row):
                        ...
                        $opts .= "<option $selected  value='" . $row->$lookup_key . "' $mandatory > " . $val . " </option> ";
                    endforeach;
                } else {
                    $opt = explode("|", $option['lookup_query']);
                    $opts = '';
                    for ($i = 0; $i < count($opt); $i++) {
                        ...
                        if ($value == ltrim(rtrim($opt[0])))
                            $selected = 'selected="selected"';
                        $row = explode(":", $opt[$i]);
                        $opts .= "<option $selected value ='" . trim($row[0]) . "' > " . Lang::get('core.' . $row[1]) . " </option> ";
                    }
                }
            ...
            case 'radio';
                ...
                for ($i = 0; $i < count($opt); $i++) {
                    $checked = '';
                    $row = explode(":", $opt[$i]);
                    $opts .= "<option value ='" . $row[0] . "' > " . Lang::get('core.' . $row[1]) . " </option> ";
                }
                $form = "<select name='$field{$bulk}' class='form-control' $mandatory ><option value=''>  --  " . Lang::get('core.Select') . "  -- </option>$opts</select>";
                break;
        }
        ...
        if (Session::has('msgstatus')):
            ...
            <script type="text/javascript">
                $(document).ready(function () {
                    toastr.<?php echo $status; ?>("<?php echo Session::get('success'); ?>", "<?php echo Session::get('messagetext'); ?>");
                    toastr.options = {
                        "closeButton": true,
                        "debug": false,
...
//        echo $files ; die;
    ...
    public static function gridDisplay($val, $field, $arr) {
        ...
        if (isset($arr['valid']) && $arr['valid'] == 1) {
            ...
            if (count($Q) >= 1) {
                ...
                $v = '';
                $fi1 = isset($fields[0])? $fields[0] : '';
                $fi2 = isset($fields[1])? $fields[1] : '';
                $fi3 = isset($fields[2])? $fields[2] : '';
                $v .= (isset($fields[0]) && $fields[0] != '' ? $row->$fi1 . ' ' : '');
                $v .= (isset($fields[1]) && $fields[1] != '' ? $row->$fi2 . ' ' : '');
                $v .= (isset($fields[2]) && $fields[2] != '' ? $row->$fi3 . ' ' : '');
    ...
    public static function addNotification($notifier_id, $notified_id, $subject, $link) {
        ...
        $data2 = array(
            ...
            'link' => $link,
            'seen' => 0,
            'entry_user' => $notifier_id,
            'created_at' => date("Y-m-d H:i:s"),
            'updated_at' => date("Y-m-d H:i:s"),
        );

        \DB::table('tb_notifications')->insert($data2);


        // then send mail to notified 
        $Notifier = \DB::table('tb_users')->where('id', '=', $notifier_id)->first();
        $Notified = \DB::table('tb_users')->where('id', '=', $notified_id)->first();


        // to get request on the fly
        // $request = \Illuminate\Http\Request::capture();
        //   $message_link = $request->root() . DIRECTORY_SEPARATOR . $link;    // this make error if there is file input in the $request 
        $message_link = url('/') . DIRECTORY_SEPARATOR . $link;
        // send mail
...
// echo 'From: ' . $Notifier->email . ' TO:  '.$Notified->email; die ;

        $headers = 'MIME-Version: 1.0' . "\r\n";
        $headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
        $headers .= 'From: ' . $Notifier->email;

        @mail($Notified->email, $subject, $message, $headers);
    }

    public static function sendMail($subject, $form_mail, $to_mail, $total_cost) {
        // send mail
        $message = '<!DOCTYPE html>
					<html lang="en-US">
						<head>
...
// echo 'From: ' . $Notifier->email . ' TO:  '.$Notified->email; die ;
    ...
    public static function inquiryaccess_departmentmanager($department_id) {
        $access_view = 0;
        $manager_id = \DB::table('tb_users')
                ->join('tb_departments', 'tb_departments.manager', '=', 'tb_users.employee_id')
                ->where('tb_departments.id', $department_id)
                ->value('tb_users.id');
        if ($manager_id == \Session::get('uid')) {
            $access_view = 1;
        }
        return $access_view;
    }

    public static function inquiryaccess_from($from) {
        $access_view = 0;
        if ($from == \Session::get('uid')) {
            $access_view = 1;
        }
        return $access_view;
    }

    public static function inquiryaccess_to($to, $department_id) {
        $access_view = 0;
        if ($to == 0) {
            $data['employee_view'] = \DB::table('tb_employees')
                    ->select('user_id')
                    ->where('department_id', $department_id)
                    ->where('user_id', '<>', '')
                    ->get();
            foreach ($data['employee_view'] as $employeerow) {
                if (\Session::get('uid') == $employeerow->user_id) {
                    $access_view = 1;
                    break;
                }
            }
    ...
    public static function getnotifications() {
        ...
        $notifications = array();
        $notifications['notification'] = \DB::table('tb_notifications')
                ->select('id', 'subject', 'link')
                ->where('seen', 0)
                ->where('notified_id', $currentUserId)
                ->get();
        return $notifications['notification'];
    }

    public static function getnotificationstotal() {
        $currentUserId = \Session::get('uid');
        $notification_total = \DB::table('tb_notifications')
                ->select('subject', 'link')
                ->where('seen', 0)
                ->where('notified_id', $currentUserId)
                ->count();
    ...
    public static function getNames($ids, $table, $field) {
        $ids_as_name = "";
        ...
        if (count($ids_array) > 0) {
            foreach ($ids_array as $key => $id) {
                if (isset($id) && $id > 0) {
                    $results = DB::select(DB::raw("SELECT * FROM {$table} WHERE id = {$id} "));
                    $new_array[] = $results[0]->$field;
                }
            }
            $ids_as_name = implode(",", $new_array);
        }
        return $ids_as_name;
    }
    ...
    public static function getDepartment($department_id) {
        $emp_fname = "";
        $results = DB::select(DB::raw("SELECT * FROM tb_departments WHERE id = {$department_id} "));
        if ($results) {
            $manger_id = $results[0]->manager;
            if ($manger_id) {
                $results2 = DB::select(DB::raw("SELECT * FROM tb_employees WHERE id = {$manger_id} "));
                if ($results2) {
                    $emp_fname = $results2[0]->fname;
                }
            }
        }
    ...
    public static function getCurrentController() {
        ...
        $arra = explode('@', $controller);
        $arr2 = explode('Controller', $arra[0]);


        return strtolower($arr2[0]);
    }

    public static function employeesonjob() {
        $total = 0;
        $employees = \DB::table('tb_project_employees')
                ->where('job_start_date', '<=', date('Y-m-d H:i:s'))
                ->where('job_end_date', '>=', date('Y-m-d H:i:s'))
                ->select('employees_ids')
                ->get();
        if (!empty($employees)) {
            foreach ($employees as $erow) {
                if (strstr($erow->employees_ids, ',')) {
                    $employee_array = explode(",", $erow->employees_ids);
    ...
    public static function latestinquiries() {

        $employee_id = \DB::table('tb_employees')->where('user_id', \Session::get('uid'))->value('id');
    ...
    public static function totalemployees() {
        ...
        $total_employees = \DB::table('tb_employees')->count();

        return $total_employees;
    }

    public static function Remainingdays($id) {
        $remaining = \DB::table('tb_employees')->where('id', $id)->value('remaining_days');
        return $remaining;
    }

    // check all department approve for specific contract
    public static function checkContractDepartmentsApprove($id) {
        $contract_id = $id;
        $Contract = Contracts::findorFail($contract_id);
        $contractDeptsCount = Contractdepartments::where('contract_id', '=', $contract_id)->count();
        $approvedContractDeptsCount = Contractdepartments::where('contract_id', '=', $contract_id)->where('approve', '=', 1)->count();

        $all_departments_approve = 0;
        if ($approvedContractDeptsCount == $contractDeptsCount && $contractDeptsCount > 0) {
            $all_departments_approve = 1;
        }
        return $all_departments_approve;
    }

    public static function getMonthesDiff($contractId) {
        $Contract = Contracts::where('id', $contractId)->first();

        $d1 = new \DateTime($Contract->end_date);
        $d2 = new \DateTime(date("Y-m-d"));
        $interval = $d2->diff($d1);
        $color = "";
        if ($interval->y == 0) { // within the same year 
            if ($interval->m == 3 || $interval->m == 2) {  // less than 3 monthes or 2 monthes
                $color = "#DFBA49";
            } elseif ($interval->m <= 1) {
                $color = "#f00";
            } else {
    ...
    public static function getQuestionTranslate($QuestionId, $langId) {
        $Questionstranslation = Questionstranslation::where('question_id', $QuestionId)->where('language', $langId)->first();
    ...
    public static function getAnswerLang($questionTransId, $key, $questionsAnswerId) {
        $Answer = Answers::where('question_trans_id', $questionTransId)->where('language', $key)->where('question_answer_id', $questionsAnswerId)->first();
                if ($Answer) {
                    return $Answer;
                }else{
                     return "";
